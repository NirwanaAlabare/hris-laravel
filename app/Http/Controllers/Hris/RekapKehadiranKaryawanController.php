    ...
    public function excel_rekap_absen(){
        ...
        foreach($sub_dept_names as $key=>$value){
            $all_dept[$key]=[
                ...
                'employee'=>MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('sub_dept_name',$value->sub_dept_name)->count(),
                'present_employee'=>MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('sub_dept_name',$value->sub_dept_name)->where('absen_masuk_kerja','!=',null)->count(),
                'absent_employee'=>MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('sub_dept_name',$value->sub_dept_name)->where('absen_masuk_kerja',null)->count(),
                'resign_employee'=>MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('sub_dept_name',$value->sub_dept_name)->where('tanggal_resign',date('Y-m-d'))->count(),
                'recruitment_employee'=>MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('sub_dept_name',$value->sub_dept_name)->where('join_date',date('Y-m-d'))->count(),
            ];
        }
        ...
        foreach($dept_names as $key=>$value){
            if(MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('absen_masuk_kerja','!=',null)->where('department_name',$value)->count()==0){
                ...
                $persentase_present_employee=round(((MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('absen_masuk_kerja','!=',null)->where('department_name',$value)->count()/$all_present_employee_dept_names)*100),1);
            }
            $all_dept_direct[$key]=[
                'dept_name'=>$value,
                'active_employee'=>MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('status_absen','!=','R')->where('department_name',$value)->count(),
                'persentase_active_employee'=>round(((MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('status_absen','!=','R')->where('department_name',$value)->count()/$all_active_employee_dept_names)*100),1),
                'present_employee'=>MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('absen_masuk_kerja','!=',null)->where('department_name',$value)->count(),
        ...
        foreach($dept_names_2 as $key=>$value){
            if(MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('absen_masuk_kerja','!=',null)->where('department_name',$value)->count()==0){
                ...
                $persentase_present_employee3=round(((MasterDataAbsenKehadiran::where('tanggal_berjalan',date('Y-m-d'))->where('absen_masuk_kerja','!=',null)->where('department_name',$value)->count()/$all_present_employee_dept_names)*100),1);
            }
