...
class RekapPerhitunganPayrollController extends AdminBaseController
{
    ...
    public function proses_payroll_harian(){
        ...
        foreach($master_absen as $key=>$value){
            ...
            if(count($value->koreksi_upah->where('tanggal_koreksi',$value->tanggal_berjalan)->where('jenis_koreksi','!=',2)->where('is_verifikasi_acc', 1))>0){
                $koreksi_upah=$value->koreksi_upah->where('tanggal_koreksi',$value->tanggal_berjalan)->where('jenis_koreksi','!=',2)->where('is_verifikasi_acc', 1)->sum('jumlah_rp_potongan');
            }
