...
class RekapPerhitunganPayrollController extends AdminBaseController
{
    ...
    public function ajax_data(Request $request)
    {
        ...
        if (request()->ajax()) {
            ...
            $data = array();
            foreach ($query as $q) {
                if ($q->lembur1_rupiah == 0) {
                    $lembur1 = 0;
                } else {
                    $lembur1 = number_format($q->lembur1_rupiah, 2, ',', '.');
                }
                if ($q->lembur2_rupiah == 0) {
                    $lembur2 = 0;
                } else {
                    $lembur2 = number_format($q->lembur2_rupiah, 2, ',', '.');
                }
                if ($q->lembur3_rupiah == 0) {
                    $lembur3 = 0;
                } else {
                    $lembur3 = number_format($q->lembur3_rupiah, 2, ',', '.');
                }
                if ($q->lembur4_rupiah == 0) {
                    $lembur4 = 0;
                } else {
                    $lembur4 = number_format($q->lembur4_rupiah, 2, ',', '.');
                }
                if ($q->koreksi_upah_rupiah == 0) {
                    $koreksi_upah_rupiah = 0;
                } else {
                    $koreksi_upah_rupiah = number_format($q->koreksi_upah_rupiah, 2, ',', '.');
                }
                if ($q->koreksi_potongan_rupiah == 0) {
                    $koreksi_potongan_rupiah = 0;
                } else {
                    $koreksi_potongan_rupiah = number_format($q->koreksi_potongan_rupiah, 2, ',', '.');
                }
                if ($q->potongan_kehadiran_rupiah == 0) {
                    $potongan_kehadiran_rupiah = 0;
                } else {
                    $potongan_kehadiran_rupiah = number_format($q->potongan_kehadiran_rupiah, 2, ',', '.');
                }
                if ($q->potongan_dtpc_rupiah + $q->potongan_iks_rupiah == 0) {
                    $potongan_jam = 0;
                } else {
                    $potongan_jam = number_format($q->potongan_dtpc_rupiah + $q->potongan_iks_rupiah, 2, ',', '.');
                }
    ...
    public function get_payroll_department()
    {
        ...
        foreach ($departement as $key => $value) {
            ...
            // dd($payroll->toArray());
            if ($payroll->sum('upah_bruto_rupiah') == 0) {
                $bruto = 0;
            } else {
                $bruto = number_format($payroll->sum('upah_bruto_rupiah'), 2, ',', '.');
            }
            $bruto_int = $payroll->sum('upah_bruto_rupiah');
            if ($payroll->sum('pph21') == 0) {
                $pph21 = 0;
            } else {
                $pph21 = number_format($payroll->sum('pph21'), 2, ',', '.');
            }
            $pph21_int = $payroll->sum('pph21');
            if ($payroll->sum('upah_neto_rupiah') == 0) {
                $upah_neto_rupiah = 0;
            } else {
                $upah_neto_rupiah = number_format($payroll->sum('upah_neto_rupiah'), 2, ',', '.');
            }
            $upah_neto_rupiah_int = ceil($payroll->sum('upah_neto_rupiah') / 100) * 100;
            if ($payroll->sum('total_bpjs_tk') == 0) {
                $total_bpjs_tk = 0;
            } else {
                $total_bpjs_tk = number_format($payroll->sum('total_bpjs_tk'), 2, ',', '.');
            }
            $total_bpjs_tk_int = $payroll->sum('total_bpjs_tk');
            if ($payroll->sum('total_bpjs_ks') == 0) {
                $total_bpjs_ks = 0;
            } else {
                $total_bpjs_ks = number_format($payroll->sum('total_bpjs_ks'), 2, ',', '.');
            }
            ...
            if ($payroll_before->sum('total_upah_thp_rupiah') == 0) {
                $jumlah_sebelum = 0;
            } else {
                $jumlah_sebelum = number_format($payroll_before->sum('total_upah_thp_rupiah'), 0, ',', '.');
            }
            // $jumlah_sebelum_int=$payroll_before->sum('total_upah_thp_rupiah');


            if ($payroll->sum('bpjs_tk_jkm_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkm_rupiah') + $payroll->sum('bpjs_tk_jkk_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkk_rupiah') + $payroll->sum('bpjs_tk_jht_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jht_rupiah') + $payroll->sum('bpjs_tk_jpn_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jpn_rupiah') + $payroll->sum('bpjs_tk_jkn_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkn_rupiah') == 0) {
                $bpjs_tk_perusahaan = 0;
            } else {
                $bpjs_tk_perusahaan = $payroll->sum('bpjs_tk_jkm_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkm_rupiah') + $payroll->sum('bpjs_tk_jkk_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkk_rupiah') + $payroll->sum('bpjs_tk_jht_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jht_rupiah') + $payroll->sum('bpjs_tk_jpn_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jpn_rupiah') + $payroll->sum('bpjs_tk_jkn_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkn_rupiah');
            }
            ...
            $data[$key] = [
                ...
                'total_bpjs_tk_all' => (float)$payroll->sum('total_bpjs_tk') + $bpjs_tk_perusahaan,
                'bpjs_ks_perusahaan' => $bpjs_ks_perusahaan,
                'total_bpjs_ks_all' => (float)$payroll->sum('total_bpjs_ks') + $bpjs_ks_perusahaan,
                'periode' => $year . '-' . $month
            ];
        }
        $arrayData = [];
        ...
        if ($bruto_total == 0) {
            ...
            $bruto_total = number_format($bruto_total, 2, ',', '.');
        }
        ...
        if ($total_bpjs_tk_total == 0) {
            $total_bpjs_tk_total = 0;
        } else {
            $total_bpjs_tk_total = number_format($total_bpjs_tk_total, 2, ',', '.');
        }
        $total_bpjs_ks_total = array_sum(array_column($arrayData, 'total_bpjs_ks_int'));
        if ($total_bpjs_ks_total == 0) {
            $total_bpjs_ks_total = 0;
        } else {
            $total_bpjs_ks_total = number_format($total_bpjs_ks_total, 2, ',', '.');
        }
        $potongan_total = array_sum(array_column($arrayData, 'potongan_int'));
        if ($potongan_total == 0) {
            $potongan_total = 0;
        } else {
            $potongan_total = number_format($potongan_total, 2, ',', '.');
        }
        ...
        if ($jumlah_sebelum_total == 0) {
            $jumlah_sebelum_total = 0;
        } else {
            $jumlah_sebelum_total = number_format($jumlah_sebelum_total, 2, ',', '.');
        }
        ...
        if ($selisih_gaji_total == 0) {
            $selisih_gaji_total = 0;
        } else {
            $selisih_gaji_total = number_format($selisih_gaji_total, 2, ',', '.');
        }
        $grand_total = [
            'id_department' => '',
    ...
    public function export_excel_summary_department()
    {
        ...
        foreach ($departement as $key => $value) {
            ...
            if (ceil($payroll_before->sum('total_upah_thp_rupiah') / 100) * 100 == 0) {
                $jumlah_sebelum = 0;
            } else {
                $jumlah_sebelum = number_format(ceil($payroll_before->sum('total_upah_thp_rupiah') / 100) * 100, 0, ',', '.');
            }
            if ($payroll->sum('bpjs_tk_jkm_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkm_rupiah') + $payroll->sum('bpjs_tk_jkk_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkk_rupiah') + $payroll->sum('bpjs_tk_jht_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jht_rupiah') + $payroll->sum('bpjs_tk_jpn_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jpn_rupiah') + $payroll->sum('bpjs_tk_jkn_perusahaan_rupiah') + $payroll->sum('bpjs_tk_jkn_rupiah') == 0) {
                $bpjs_tk_perusahaan = 0;
            } else {
        ...
        if ($upah_neto_rupiah_int_total == 0) {
            ...
            $upah_neto_rupiah_int_total = number_format($upah_neto_rupiah_int_total, 2, ',', '.');
        }
        ...
        if ($jumlah_total == 0) {
            ...
            $jumlah_total = number_format($jumlah_total, 2, ',', '.');
        }
...
//     }
    ...
    public function proses_payroll_harian()
    {
        ...
        if (request()->enroll_id) {
            ...
            $inRnrollid1 = 'AND a.enroll_id in (' . $enroll_id_string . ')';
            $inRnrollid2 = '  (' . $enroll_id_string . ')';
        }
    $master_absen = MasterDataAbsenKehadiran::from('master_data_absen_kehadiran as a')

        // JOIN employee_atribut (biar bisa dipakai di COALESCE)
        ->join('employee_atribut as c', 'a.enroll_id', '=', 'c.enroll_id')

        // 🔥 JOIN grading_history
        ->leftJoin('grading_history as gh', function ($join) {
            $join->on('a.enroll_id', '=', 'gh.enroll_id')
                ->whereRaw("
                    a.tanggal_berjalan BETWEEN
                    STR_TO_DATE(SUBSTRING_INDEX(gh.periode_payroll,' s/d ',1),'%Y-%m-%d')
                    AND
                    STR_TO_DATE(SUBSTRING_INDEX(gh.periode_payroll,' s/d ',-1),'%Y-%m-%d')
                ");
        })

        // 🔥 JOIN rekap_perhitungan_payroll
        ->leftJoin('rekap_perhitungan_payroll as rp', function ($join) {
            $join->on('a.enroll_id', '=', 'rp.enroll_id')
                ->whereRaw("
                    a.tanggal_berjalan BETWEEN
                    STR_TO_DATE(SUBSTRING_INDEX(rp.periode_kehadiran,' s/d ',1),'%Y-%m-%d')
                    AND
                    STR_TO_DATE(SUBSTRING_INDEX(rp.periode_kehadiran,' s/d ',-1),'%Y-%m-%d')
                ");
        })

        // 🔥 JOIN grading_salary (COALESCE)
        ->leftJoin('grading_salary as d', function ($join) {
            $join->whereRaw("
                d.kode_grade = COALESCE(gh.kode_grade_baru, rp.kode_grade, c.kode_grade)
            ")
            ->whereRaw("
                YEAR(a.tanggal_berjalan) = SUBSTR(d.periode_umk, 1, 4)
            ");
        })

        // FILTER
        ->whereBetween('a.tanggal_berjalan', [$first_date, $last_date])
        ->when(!empty($enroll_id), function ($query) use ($enroll_id) {
        $query->whereIn('a.enroll_id', $enroll_id);
        })

        // 🔥 SELECT TAMBAHAN
        ->select(
            'a.*',
            'd.kode_grade as final_grade',
            'd.salary_bulanan'
        )

        // 🔥 RELASI ELOQUENT (INI YANG KAMU MAU)
        ->with([
            'rekap_lembur' => function ($query) use ($first_date, $last_date) {
                $query->whereBetween('tanggal_berjalan', [$first_date, $last_date]);
            },
            'employee_atribut',
            'employee_atribut.employee_bpjs',
            'employee_atribut.group_department',
            'koreksi_upah',
            'koreksi_potongan'
        ])
        // dd($master_absen->get());
        ...
        //     dd($master_absen->toSql(), $master_absen->get());
        //     // ->get();
        ...
        $rekap_lembur_gabungan = DB::select("SELECT r4.tanggal_berjalan,r4.capai_target,r4.kode_hari,r4.mulai_jam_kerja,r4.akhir_jam_kerja,r4.status_absen,r4.nomor_form_lembur,r4.kode_grade,r4.salary_bulanan,
                                                    r4.enroll_id,r4.finish_in,r4.finish_out,r4.absen_masuk_kerja,r4.absen_pulang_kerja,
                                                    FLOOR(r4.selisih_detik/3600) selisih_jam,
                                                    FLOOR(MOD(r4.selisih_detik,3600)/60) selisih_menit,
                                                    MOD(MOD(r4.selisih_detik,3600),60) selisih_detik,
                                                        CONCAT(
                                                            LPAD(FLOOR(r4.selisih_detik/3600),2,'0'),':',
                                                            LPAD(FLOOR(MOD(r4.selisih_detik,3600)/60),2,'0'),':',
                                                            LPAD(MOD(MOD(r4.selisih_detik,3600),60),2,'0')
                                                        ) AS final_total,
                                                    r4.jumlah_jam_lembur,
                                                    r4.jumlah_jam_istirahat_lembur
                                            FROM ( SELECT r3.*,
                                                            CASE
                                                                WHEN (r3.jumlah_menit_absen_pc != 0
                                                                    AND r3.mulai_jam_lembur >= r3.akhir_jam_kerja)
                                                                    OR r3.absen_masuk_kerja IS NULL
                                                                    OR r3.absen_pulang_kerja IS NULL
                                                                THEN 0
                                                                ELSE GREATEST(r3.jam_2 - r3.jam_1,0)
                                                            END AS selisih_detik
                                            FROM ( SELECT r2.*,
                                                            CASE
                                                                WHEN r2.jam_2_raw < r2.jam_1
                                                                THEN r2.jam_2_raw + 86400
                                                                ELSE r2.jam_2_raw
                                                            END AS jam_2
                                            FROM ( SELECT r1.*,
                                                    TIME_TO_SEC(r1.finish_in)  AS jam_1,
                                                    TIME_TO_SEC(r1.finish_out) AS jam_2_raw
                                            FROM ( SELECT
                                                            a.enroll_id,
                                                            a.tanggal_berjalan,
                                                            a.kode_hari,
                                                            ...
                                                            a.akhir_jam_kerja,
                                                            a.status_absen,
                                                            a.jumlah_menit_absen_pc,

                                                            b.nomor_form_lembur,
                                                            b.capai_target,

                                                            /* ============================= */
                                                            /* PENENTUAN KODE GRADE BARU */
                                                            /* ============================= */
                                                            -- if(gh.kode_grade_baru is not null, gh.kode_grade_baru,c.kode_grade) kode_grade,
                                                            COALESCE(
																																		gh.kode_grade_baru,
																																		rp.kode_grade,
																																		c.kode_grade
                                                            ...
                                                            CASE
                                                                WHEN a.mulai_jam_kerja IS NULL
                                                                THEN GREATEST(
                                                                    STR_TO_DATE(SUBSTRING(a.absen_masuk_kerja,1,5),'%H:%i'),
                                                                    STR_TO_DATE(SUBSTRING(b.mulai_jam_lembur,12,5),'%H:%i')
                                                                )
                                                                ELSE
                                                                CASE
                                                                    WHEN STR_TO_DATE(SUBSTRING(b.mulai_jam_lembur,12,5),'%H:%i')
                                                                        <= STR_TO_DATE(SUBSTRING(a.mulai_jam_kerja,1,5),'%H:%i')
                                                                    THEN GREATEST(
                                                                        STR_TO_DATE(SUBSTRING(b.mulai_jam_lembur,12,5),'%H:%i'),
                                                                        STR_TO_DATE(SUBSTRING(a.absen_masuk_kerja,1,5),'%H:%i')
                                                                    )
                                                            ...
                                                            a.absen_masuk_kerja,
                                                            a.absen_pulang_kerja,
                                                            b.jumlah_jam_lembur,
                                                            b.jumlah_jam_istirahat_lembur,
                                                            b.mulai_jam_lembur
                                            FROM master_data_absen_kehadiran a
                                            INNER JOIN data_lembur b
                                                ON a.enroll_id=b.enroll_id
                                                AND a.tanggal_berjalan=b.tanggal_berjalan $inRnrollid1
                                            INNER JOIN employee_atribut c
                                                ON a.enroll_id=c.enroll_id
                                            /* ============================= */
                                            /* JOIN KE GRADING HISTORY */
                                            /* ============================= */
                                            LEFT JOIN grading_history gh
                                                ON a.enroll_id = gh.enroll_id AND a.tanggal_berjalan BETWEEN STR_TO_DATE(SUBSTRING_INDEX(gh.periode_payroll,' s/d ',1),'%Y-%m-%d') AND
												STR_TO_DATE(SUBSTRING_INDEX(gh.periode_payroll,' s/d ',-1),'%Y-%m-%d')
											left join rekap_perhitungan_payroll rp on a.enroll_id = rp.enroll_id and a.tanggal_berjalan BETWEEN STR_TO_DATE(SUBSTRING_INDEX(rp.periode_kehadiran,' s/d ',1),'%Y-%m-%d')
											AND STR_TO_DATE(SUBSTRING_INDEX(rp.periode_kehadiran,' s/d ',-1),'%Y-%m-%d')
											right join grading_salary d on d.kode_grade = COALESCE(gh.kode_grade_baru,rp.kode_grade,c.kode_grade) and YEAR(a.tanggal_berjalan) = SUBSTR( d.periode_umk, 1, 4 )
                                            WHERE
                                                a.tanggal_berjalan BETWEEN '$first_date' AND '$last_date'
                                                AND a.nomor_form_lembur IS NOT NULL
                                                        ) r1
                                                    ) r2
                                                ) r3
                                            ) r4;");
        // dd($rekap_lembur_gabungan);
        foreach ($rekap_lembur_gabungan as $key => $value) {
         ...
         $map_grade = [];
        foreach ($rekap_lembur_gabungan as $row) {
            $key = $row->enroll_id . '_' . $row->tanggal_berjalan;
            $map_grade[$key] = [
                'kode_grade' => $row->kode_grade,
                'salary_bulanan' => $row->salary_bulanan
            ];
        }
        ...
        foreach ($master_absen as $key => $value) {
            ...
            // $map_key = $enroll_id_karyawan . '_' . $value->tanggal_berjalan;

            // $salary_bulanan = 0;
            $kode_grade = $value->final_grade;

            // if (isset($map_grade[$map_key])) {

            //     // ambil dari grading history
            //     $salary_bulanan = $map_grade[$map_key]['salary_bulanan'];
            //     $kode_grade = $map_grade[$map_key]['kode_grade'];

            // } else {

            //     // fallback ke employee_atribut
            //     $tahun_berjalan = substr($value->tanggal_berjalan, 0, 4);

            //     $grading_salary = $value->employee_atribut->grading_salary
            //         ->filter(function ($item) use ($tahun_berjalan) {
            //             return substr($item->periode_umk, 0, 4) == $tahun_berjalan;
            //         })
            //         ->first();

            //     if ($grading_salary) {
            //         $salary_bulanan = $grading_salary->salary_bulanan;
            //         $kode_grade = $grading_salary->kode_grade ?? null;
            //     }
            ...
            // }
            $grading_salary = GradingSalary::where('kode_grade', $kode_grade)
                ->whereRaw('LEFT(periode_umk, 4) = ?', [$tahun_berjalan])
                ->first();
            // dd($kode_grade,$tahun_berjalan,$grading_salary->insentif/21);
            if ($grading_salary && $grading_salary->insentif) {
                $insentif_kehadiran = $grading_salary->insentif / 21;
            }
            ...
            $thr = 0;
            if ($selisih_hari > 30) {
                // $grading_salary = $value->grading_history->grading_salary->filter(function ($item) use ($tahun_berjalan) {
                //     return substr($item->periode_umk, 0, 4) == $tahun_berjalan;
                // })->first();

                $thr =  $value->salary_bulanan / 12 / $jumlah_hari_kerja;
            }
            ...
            if ($security->where('enroll_id', $value->enroll_id)->count()) {
                $gaji_perhari =   $value->salary_bulanan / 25;
                ...
                $bruto = ($value->mulai_jam_kerja != null) ? (($gaji_perhari_total + $seniority_allowance_total + $insentif_kehadiran_total + $koreksi_upah + $insentif_jabatan_terpakai) - ($koreksi_potongan + $potongan_permenit + $potongan_perhari)) : (($potongan_permenit + $potongan_perhari));
                ...
                $gaji_perhari =  $value->salary_bulanan / $jumlah_hari_kerja;
