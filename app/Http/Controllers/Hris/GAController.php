...
class GAController extends AdminBaseController
{
    ...
    public function xupdate_car_request_user()
    {
        $loggedAdmin = Auth::guard('admin')->user();
    ...
    public function update_car_request_user(Request $request)
    {
        $admin = Auth::guard('admin')->user();
        $id = $request->id;

        $permintaan = PermintaanTransportasi::findOrFail($id);

        // 1. Persiapan Data Lama (Old Data)
        $oldDataComplete = [
            'main_table' => $permintaan->toArray(),
            'details'    => TujuanTransportasi::where('permintaan_transportasi_id', $id)->get()->toArray()
        ];
        ...
        try {
            ...
            // 4. Update Detail Tujuan (Re-sync)
            // Ambil ID dari record lama sebelum didelete untuk pelacakan
            $oldDetailIds = TujuanTransportasi::where('permintaan_transportasi_id', $id)->pluck('id')->toArray();

            TujuanTransportasi::where('permintaan_transportasi_id', $id)->delete();

            $newDetails = [];
            if ($request->has('tujuan_pemberangkatan_array')) {
                foreach ($request->tujuan_pemberangkatan_array as $key => $value) {
                    $date = str_replace('/', '-', $request->tanggal_pemberangkatan_array[$key]);

                    $detail = TujuanTransportasi::create([
                        'permintaan_transportasi_id' => $id,
                        'tujuan_id'                  => $key,
                        'subdistrict'                => $request->desa_array[$key] ?? null,
                        'instansi'                   => $request->instansi_array[$key] ?? null,
                        'detail_alamat'              => $request->detail_alamat_array[$key] ?? null,
                        'tanggal_kedatangan'         => date('Y-m-d', strtotime($date)),
                        'jam_kedatangan'             => $request->jam_pemberangkatan_array[$key] ?? null,
                        'jarak_tempuh'               => $request->jarak_tempuh_array[$key] ?? null,
                        'tujuan_pemberangkatan'      => $value,
                        'jenis_barang'               => $request->jenis_barang_array[$key] ?? null,
                        'quantity'                   => $request->quantity_array[$key] ?? 0,
                        'satuan'                     => $request->satuan_array[$key] ?? null,
                        'nama_penerima'              => $request->nama_penerima_array[$key] ?? null,
                        'keterangan_barang'          => $request->keterangan_barang_array[$key] ?? null,
                        'nama_tamu'                  => $request->nama_tamu_array[$key] ?? null,
                        'nomor_hp_tamu'              => $request->nomor_hp_tamu_array[$key] ?? null,
                        'karyawan_dinas_luar'        => $request->karyawan_dinas_luar_array[$key] ?? null,
                    ]);
                    $newDetails[] = $detail->toArray();
                }
            }

            // 5. Persiapan Data Baru untuk Log
            $newDataComplete = [
                'main_table' => $permintaan->fresh()->toArray(),
                'details'    => $newDetails
            ];

            // 6. Logika Deteksi Data yang Dihapus
            // Membandingkan data lama yang didelete dengan yang baru dibuat
            // Jika Anda ingin melihat apa saja yang dihapus berdasarkan nilai kolom:
            $deletedDetails = [];
            foreach ($oldDataComplete['details'] as $oldItem) {
                $found = false;
                foreach ($newDetails as $newItem) {
                    // Kecocokan ditentukan oleh kombinasi field unik
                    if (
                        (string)$oldItem['tujuan_id'] == (string)$newItem['tujuan_id'] &&
                        (string)$oldItem['subdistrict'] == (string)$newItem['subdistrict'] &&
                        (string)$oldItem['instansi'] == (string)$newItem['instansi'] &&
                        (string)$oldItem['tujuan_pemberangkatan'] == (string)$newItem['tujuan_pemberangkatan']
                    ) {
                        $found = true;
                        break;
                    }
                ...
                if (!$found) {
                    $deletedDetails[] = $oldItem;
                }
            }
            $newDataComplete['deleted_details'] = $deletedDetails;

            // 7. Simpan Activity Log
            \DB::table('activity_logs')->insert([
                'action_by_id'   => $admin->id,
                'action_by_name' => $admin->name,
                'table_name'     => 'permintaan_transportasi',
                'log_name'       => 'Update Permintaan Transportasi',
                'record_id'      => $id,
                'action'         => 'update',
                'old_data'       => json_encode($oldDataComplete),
                'new_data'       => json_encode($newDataComplete),
                'created_at'     => now(),
                'updated_at'     => now()
            ]);

            DB::commit();
            return response()->json(['status' => 'success', 'message' => 'Data dan Log berhasil diperbarui!'], 200);
        } catch (\Exception $e) {
            DB::rollBack();
    ...
    public function xupdate_car_request_administrator()
    {
        $loggedAdmin = Auth::guard('admin')->user();
        $created_by = $loggedAdmin->enroll_id;
        $id = request()->id;
    ...
    public function update_car_request_administrator(Request $request)
    {
        $admin = Auth::guard('admin')->user();
        $id = $request->id;

        $permintaan = PermintaanTransportasi::find($id);
        ...
        try {
            ...
            // 3. Update Detail (Driver & Vehicle)
            if ($request->has('array_driver')) {
                foreach ($request->array_driver as $key => $value) {
                    TujuanTransportasi::where('tujuan_id', $key)
                        ->where('permintaan_transportasi_id', $id) // Tambahkan constraint ID untuk keamanan
                        ->update([
                            'driver'      => $value,
                            'vehicle'     => $request->array_vehicle[$key] ?? null,
                            'status'      => $request->array_status[$key] ?? null,
                            'alternative' => $request->array_alternative[$key] ?? null
                        ]);
                }
            }

            // 4. Snapshot Data Baru
            $newData = [
                'main'    => $permintaan->fresh()->toArray(),
                'details' => TujuanTransportasi::where('permintaan_transportasi_id', $id)->get()->toArray()
            ];
            ...
            DB::commit();
            return response()->json(['status' => 'success', 'message' => 'Status berhasil diupdate dan dicatat dalam log.'], 200);
        } catch (\Exception $e) {
            DB::rollBack();
            \Log::error("Gagal update status admin: " . $e->getMessage());
    ...
    public function store_car_request(Request $request)
    {
        $admin = Auth::guard('admin')->user();
        ...
        try {
            ...
            // 1. Simpan Data Utama
            $permintaan = PermintaanTransportasi::create([
                'enroll_id'              => $request->employee,
                'id_desa'                => $request->sub_districts,
                'instansi'               => $request->instansi,
                'detail_alamat'          => $request->detail_alamat,
                ...
                'jam_pemberangkatan'     => $request->jam_pemberangkatan,
                'status'                 => 0,
                'created_by'             => $admin->enroll_id
            ]);

            // 2. Simpan Detail Tujuan dan kumpulkan ke dalam array untuk log
            $createdDetails = [];
            if ($request->has('tujuan_pemberangkatan_array')) {
                foreach ($request->tujuan_pemberangkatan_array as $key => $value) {
                    $date = str_replace('/', '-', $request->tanggal_pemberangkatan_array[$key]);
                    ...
                    $detail = TujuanTransportasi::create([
                        'permintaan_transportasi_id' => $permintaan->id,
                        'tujuan_id'                  => $key,
                        'subdistrict'                => $request->desa_array[$key] ?? null,
                        'instansi'                   => $request->instansi_array[$key] ?? null,
                        'detail_alamat'              => $request->detail_alamat_array[$key] ?? null,
                    ...
                    $createdDetails[] = $detail->toArray();
                }
            }

            // 3. Susun data lengkap untuk log (struktur identik dengan update)
            $newDataComplete = [
                'main_table' => $permintaan->toArray(),
            ...
            \DB::table('activity_logs')->insert([
                ...
                'action_by_name' => $admin->name,
                'table_name'     => 'permintaan_transportasi',
                'log_name'       => 'Create Permintaan Transportasi',
                'record_id'      => $permintaan->id,
                'action'         => 'Create',
                'old_data'       => json_encode([]), // Kosong untuk proses create
                'new_data'       => json_encode($newDataComplete),
                'created_at'     => now(),
                'updated_at'     => now()
            ]);
            ...
            DB::commit();
            return response()->json(['status' => 'success', 'message' => 'Data berhasil dibuat!'], 200);
        } catch (\Exception $e) {
            DB::rollBack();
