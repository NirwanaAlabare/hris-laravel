...
class RekapPerhitunganPayrollExportUMK2024 implements FromQuery, WithMapping, ShouldAutoSize, WithEvents, WithCustomStartCell, WithTitle,  WithColumnFormatting
{
    ...
    public function map($Data): array
    {
        ...
        $tunjangan_karyawan_rupiah_excel=RekapPerhitunganPayroll::select('tunjangan_karyawan_rupiah')->where('enroll_id',$Data->enroll_id)->where('periode_tahun_payroll','2024')->where('periode_bulan_payroll','01')->where('periode_umk',null)->pluck('tunjangan_karyawan_rupiah')[0];
        ...
        $koreksi_potongan_rupiah_excel=RekapPerhitunganPayroll::select('koreksi_potongan_rupiah')->where('enroll_id',$Data->enroll_id)->where('periode_tahun_payroll','2024')->where('periode_bulan_payroll','01')->where('periode_umk',null)->pluck('koreksi_potongan_rupiah')[0];
        $koreksi_upah_rupiah_excel=RekapPerhitunganPayroll::select('koreksi_upah_rupiah')->where('enroll_id',$Data->enroll_id)->where('periode_tahun_payroll','2024')->where('periode_bulan_payroll','01')->where('periode_umk',null)->pluck('koreksi_upah_rupiah')[0];
        ...
        if($Data->upah_bruto_rupiah == 0){ $upah_bruto_rupiah = '0';} else { $upah_bruto_rupiah = ($Data->upah_bruto_rupiah+$koreksi_upah_rupiah_excel+$tunjangan_karyawan_rupiah_excel)-$koreksi_potongan_rupiah_excel; }
        $pph21 = $Data->pph21;
        if($Data->upah_neto_rupiah == 0){ $upah_neto_rupiah = '0';} else { $upah_neto_rupiah = ($Data->upah_neto_rupiah+$koreksi_upah_rupiah_excel+$tunjangan_karyawan_rupiah_excel)-$koreksi_potongan_rupiah_excel; }
        ...
        $nominal_bpjs_tk=RekapPerhitunganPayroll::select('total_bpjs_tk')->where('enroll_id',$Data->enroll_id)->where('periode_tahun_payroll','2024')->where('periode_bulan_payroll','01')->where('periode_umk',null)->pluck('total_bpjs_tk')[0];
        if($nominal_bpjs_tk==0){$nominal_bpjs_tk='0';}else{$nominal_bpjs_tk=$nominal_bpjs_tk;}
        $nominal_bpjs_ks=RekapPerhitunganPayroll::select('total_bpjs_ks')->where('enroll_id',$Data->enroll_id)->where('periode_tahun_payroll','2024')->where('periode_bulan_payroll','01')->where('periode_umk',null)->pluck('total_bpjs_ks')[0];
        if($nominal_bpjs_ks==0){$nominal_bpjs_ks='0';}else{$nominal_bpjs_ks=$nominal_bpjs_ks;}
        $nominal_iuran_serikat=RekapPerhitunganPayroll::select('iuran_serikat_rupiah')->where('enroll_id',$Data->enroll_id)->where('periode_tahun_payroll','2024')->where('periode_bulan_payroll','01')->where('periode_umk',null)->pluck('iuran_serikat_rupiah')[0];
        $nominal_iuran_koperasi=RekapPerhitunganPayroll::select('iuran_koperasi')->where('enroll_id',$Data->enroll_id)->where('periode_tahun_payroll','2024')->where('periode_bulan_payroll','01')->where('periode_umk',null)->pluck('iuran_koperasi')[0];
    ...
    public function registerEvents(): array
    {
        ...
        return [
            AfterSheet::class => function (AfterSheet $event) use (&$counter) {
                ...
                $sheet->setCellValue('AO5', 'UMK');

                $sheet->mergeCells('AP5:AP6');
                $sheet->setCellValue('AP5', 'Seniority Allowance');

                $sheet->mergeCells('AQ5:AQ6');
                $sheet->setCellValue('AQ5', 'Upah/ Hari');

                $sheet->mergeCells('AR5:AR6');
                $sheet->setCellValue('AR5', 'Upah/ Jam');
                //kosong


                $sheet->mergeCells('AS5:AS6');
                $sheet->setCellValue('AS5', '');

                $sheet->mergeCells('AT5:AT6');
                $sheet->setCellValue('AT5', 'Gaji Pokok');

                $sheet->mergeCells('AU5:AU6');
                $sheet->setCellValue('AU5', 'Premi');
                ...
                $sheet->setCellValue('BD5', 'Rp Pot. Jam');
