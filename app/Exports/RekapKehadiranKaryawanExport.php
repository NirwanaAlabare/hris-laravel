...
class RekapKehadiranKaryawanExport implements FromQuery, WithMapping, ShouldAutoSize, WithEvents, WithCustomStartCell, WithTitle
{
    ...
    public function query()
    {
        ...
        if($searchData) {
            $whereSearchData = ' AND (upper( rkk.enroll_id ) LIKE upper( "%' . $searchData . '%" ) OR upper( employee_name ) LIKE upper( "%' . $searchData . '%" ) OR upper( sub_dept_name ) LIKE upper( "%' . $searchData . '%" ) OR upper( status_aktif ) LIKE upper( "%' . $searchData . '%" ) ) ';
        } else {
            $whereSearchData = '';
        }
