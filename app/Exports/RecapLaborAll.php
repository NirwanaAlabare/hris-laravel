...
class RecapLaborAll implements WithTitle, WithEvents, FromCollection, WithMapping, WithColumnWidths, WithColumnFormatting, WithCustomStartCell
{
    ...
    public function collection()
    {
        ...
        LEFT JOIN (
        SELECT enroll_id, tanggal_koreksi,
            SUM(CASE WHEN jenis_potongan IN (3,4, 6) THEN jumlah_rp_potongan ELSE 0 END) AS potongan_piutang,
        ...
        left join ref_absen_ijin h on a.status_absen=h.kode_absen_ijin left join data_absen_perijinan dap on a.uuid = dap.uuid_master
        LEFT JOIN grading_history gh ON a.enroll_id = gh.enroll_id and  a.tanggal_berjalan BETWEEN STR_TO_DATE(SUBSTRING_INDEX(gh.periode_payroll,' s/d ',1),'%Y-%m-%d') AND STR_TO_DATE(SUBSTRING_INDEX(gh.periode_payroll,' s/d ',-1),'%Y-%m-%d')
        LEFT JOIN rekap_perhitungan_payroll rp on a.enroll_id = rp.enroll_id and a.tanggal_berjalan BETWEEN STR_TO_DATE(SUBSTRING_INDEX(rp.periode_kehadiran,' s/d ',1),'%Y-%m-%d') AND STR_TO_DATE(SUBSTRING_INDEX(rp.periode_kehadiran,' s/d ',-1),'%Y-%m-%d')
        RIGHT JOIN grading_salary i on i.kode_grade = COALESCE(gh.kode_grade_baru,rp.kode_grade,b.kode_grade) and YEAR(a.tanggal_berjalan) = SUBSTR( i.periode_umk, 1, 4 )
                inner join dasar_pot_bpjs bpjs on REGEXP_SUBSTR(bpjs.kode_dasar_pot_bpjs, '[0-9]+')=substring(a.tanggal_berjalan,1,4) where a.tanggal_berjalan>='".$this->tanggal_awal."' and a.tanggal_berjalan<='".$this->tanggal_akhir."'".$this->inEnrollId.$this->inStatusStaff);
        //  dd($query);
    ...
    public function map($Data): array
    {
        ...
        $bruto = $Data->bruto + $Data->total_lembur_rupiah;
        $jumlah = $Data->jumlah + $Data->total_lembur_rupiah;
        $potongan_perusahaan = $Data->bpjs_tk_company + $Data->bpjs_ks_company;
        $penambahan = $Data->kompensasi + $Data->thr + $Data->konsumsi;
        $total_pembayaran = ($jumlah + $penambahan + $potongan_perusahaan);
        return [
