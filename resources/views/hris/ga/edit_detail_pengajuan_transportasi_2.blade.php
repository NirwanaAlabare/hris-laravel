...
<script>
        ...
        $.ajax({
            ...
            success: function(res){
                jQuery.each(res, function(key,val){
                    var dArr2 = val.tanggal_kedatangan.split("-"); 
                    var tanggal_kedatangan = dArr2[2]+ "/" +dArr2[1]+ "/" +dArr2[0];
                    array_provinsi.push(val.prov_id);
                    array_kota.push(val.city_id);
                    array_kecamatan.push(val.dis_id);
                    array_desa.push(val.subdistrict);
                    ...
                    array_tanggal_pemberangkatan.push(tanggal_kedatangan);
    ...
    function show_destination_list(){
        ...
        jQuery.each(array_provinsi, function(key,val){
            ...
            if(array_karyawan_dinas_luar[key]!=null){
    ...
    function saveChanges(){
        ...
        var tanggal=tanggal_pemberangkatan_asal.split('/').reverse().join('-');
        if(tanggal==''){
        ...
        jQuery.each(array_provinsi, function(key,val){
            ...
            if(array_desa[key]==''){
                document.getElementById("warning_desa_tujuan_"+key).innerHTML='Desa harus dipilih';
            }else{
                document.getElementById("warning_desa_tujuan_"+key).innerHTML='';
            }
        ...
        if(provinsi_asal_string!='' && kota_asal_string!='' && kecamatan_asal_string!='' && desa_asal_string!='' && detail_alamat_asal_string!='' && tanggal_pemberangkatan_asal!='' && jam_pemberangkatan_asal!='' && !array_provinsi.includes('') && !array_kota.includes('') && !array_kecamatan.includes('') && !array_desa.includes('') && !array_detail_alamat.includes('') && !array_tanggal_pemberangkatan.includes('') && !array_jam_pemberangkatan.includes('') && !array_tujuan_pemberangkatan.includes('') && !array_keterangan.includes('')){
            ...
            success: function(res){
                if(res=='approved'){
                    swal("", "Status permintaan transportasi telah di update oleh administrator", "error");
                }else{
