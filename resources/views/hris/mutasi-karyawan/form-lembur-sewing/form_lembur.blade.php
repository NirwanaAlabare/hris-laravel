...
@section('mainarea')
        ...
        <div class="modal-dialog modal-lg modal-dialog-scrollable" style="max-width: 85%;">
            <div class="modal-content">
                ...
                <div style="height: 100vh; overflow-y: auto; over-flow-x:none;">
                    ...
                    <form id="form_modal" name='form_modal' method='post' action="{{ route('fls.update_form_lembur') }}" onsubmit="submitForm(this, event)">
                        <div class="modal-body" id="detail">
                            <div class="form-group">
                                <label>No. Form :</label>
                                <label id = "no_form"></label>
    ...
    <div class="card card-sb">
        <div class="card-header">
            <h5 class="card-title fw-bold mb-0"><i class="fas fa-file-alt"></i> List Form Lembur Sewing</h5>
        </div>
        <div class="card-body">
            <div class="row mb-3 flex justify-content-between align-items-center">
                ...
                <div class="col-3">
                    <a href="{{ route('fls.create') }}" class="btn btn-primary position-relative w-100">
...
<script src="{{ asset('plugins/datatables-responsive/js/responsive.bootstrap4.min.js') }}"></script>
    ...
    <script>
        ...
        function storeEmployee() {
            ...
            $.ajax({
                type:"POST",
                url: '{{ route('fls.store_tambahan_data_karyawan_lembur_sewing') }}',
                data: {
                    id_c:id_c,
        ...
        function fillTheField(e){
            if(event.keyCode == 13) {
                ...
                $.ajax({
                    type:"POST",
                    url: '{{ route('fls.view_tambahan_data_karyawan_lembur_sewing') }}',
                    data: {
                        no_form:no_form,
                        enroll_id:enroll_id,
                        dept:dept
                    ...
                    success: function(res){
                        if(res=='data sudah ada'){
                            ...
                            $('#trash_'+e).empty().append('<a href="#"><span class="fa fa-trash text-danger" onClick="hapus_tambahan_karyawan_lembur_sewing('+e+','+res[0].enroll_id+')"></span></a>');
                            $('#tambah_karyawan').removeAttr('disabled');
                            $('#overtime_employee_header').removeAttr('disabled');
                            $('#overtime_employee_submit').removeAttr('disabled');
                            ...
                            $.ajax({
                                type: "post",
                                url: '{{ route('fls.get_time_from_id_data_lembur_sewing') }}',
                                data: {
                                    id: no_form,
                                },
                                success: function(res) {
        ...
        $(document).ready(function() {
            ...
            datatableFilter.innerHTML = `<span> Search : </span><input type="text" class="form-control form-control-sm" id="employee_name_filter" onkeyup="dataTableReload()">`;
        ...
        function del_tmp_enroll_id(enroll_ids){
            ...
            $.ajax({
                type: "post",
                url: '{{ route('fls.del_tmp_non_sewing_enroll_id_sewing') }}',
                data: {
                    user: user,
                    enroll_id:enroll_id
                },success: function(response) {
        ...
        function hapus_tambahan_karyawan_lembur_sewing(e,enroll_ids){
        ...
        function getdetail(id_c,id_l) {
            ...
            $("#dept_modal_input").val(id_l);
            datatableModal = $("#datatable-modal").DataTable({
                ordering: false,
                processing: true,
                serverSide: true,
                info: false,
                ...
                columnDefs: [{
                    ...
                    },
                    {
                        ...
                        render: (data, type, row, meta) => {
                            ...
                            <input type="time" size='1' class="form-control form-control-sm" id="jam_lembur_awal_rencana_`+row.enroll_id+`"
                            name="jam_lembur_awal_rencana[` + row.enroll_id + `]" value="` +
                                row.jam_lembur_awal_rencana + `"
                    ...
                    },
                    {
                        ...
                        render: (data, type, row, meta) => {
                            return `
                            <div class='d-flex gap-1 justify-content-center'>
                            <input type="time" size='1' class="form-control form-control-sm" id="jam_lembur_akhir_rencana_`+row.enroll_id+`"
        ...
        function minsToStr(t) {
            return Math.trunc(t / 60).toLocaleString('id-ID', {minimumIntegerDigits: 2,useGrouping: false})+':'+('00' + t % 60).slice(-2);
        }
        ...
        function getket(id_c) {
            let html = $.ajax({
                type: "get",
                url: '{{ route('fls.getket') }}',
                data: {
                    no_form: id_c
                },
                dataType: 'json',
                success: function(response) {
                    $("#txtket_modal").html(response.ket);
        ...
        function checkbox_enroll_id(enroll_id){
            ...
            if(checked==true){
                $.ajax({
                type: "post",
                url: '{{ route('fls.cek_data_koreksi_upah_sewing') }}',
                data: {
                    id: id,
                    no_form: no_form
                },
        ...
        function export_excel_insentif() {
            ...
            $.ajax({
                type: "get",
                url: '{{ route('fls.export_excel_insentif') }}',
                data: {
                    from: from,
                    to: to
                },
                ...
                success: function(response) {
                    {
                        ...
                        link.download = from + " sampai " +
                            to + " Laporan Checking Insentif Sewing "+Math.ceil(Math.random()*1000000)+".xlsx";
                        link.click();
        ...
        function export_excel_all() {
            ...
            $.ajax({
                type: "get",
                url: '{{ route('fls.export_excel_spl_all') }}',
                data: {
                    from: from,
                    to: to
                },
        ...
        function export_spl(id_n, no_form_n) {
            ...
            var url = 'form-lembur/export_pdf_sewing_spl?no_form='+no_form+'&id='+id;
            window.open(url, '_blank');

        }
        function export_pdf_insentif(no_form){
            var url = 'form-lembur/export_pdf_insentif?no_form='+no_form;
            window.open(url, '_blank');
        }
